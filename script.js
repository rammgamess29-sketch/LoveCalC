...
function calculateLovePercentage(name1, name2) {
  // Create a unique hash from both names
  ...
  // Generate percentage (40-100 range for better UX)
  let percentage = Math.abs(hash) % 61 + 40;

  // Special combinations for fun
  const specialCombos = {
    'romeo juliet': 100,
    'juliet romeo': 100,
    'adam eve': 95,
  ...
  const comboKey = name1.toLowerCase() + ' ' + name2.toLowerCase();
  if (specialCombos[comboKey]) {
    percentage = specialCombos[comboKey];
  }
...
function displayResults(name1, name2, percentage) {
  ...
  const {
    message,
    emojis
  } = getMessageAndEmojis(percentage);
